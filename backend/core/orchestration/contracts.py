...
class Slide(BaseModel):
    ...
    canvas_template: Optional[str] = Field(default=None, description="Fabric canvas template ID e.g. 'aurora-hook', 'aurora-content-0'. Auto-set by carousel_generator.")
...
class ContentResponse(BaseModel):
    ...
    errors: list[str] = Field(default_factory=list)
    blog_post_title:     str = Field(default="", description="LLM-crafted SEO title from BlogPostDocument")
    ...
    blog_post_json_path: str = Field(default="", description="Path to validated blog_post.json on disk")
