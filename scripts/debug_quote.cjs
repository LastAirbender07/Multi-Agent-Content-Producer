...
(async () => {
  ...
  const tile = p.locator("[data-slide-type='aurora-compact-quote']").first();
  ...
  const objs = await p.evaluate(() => {
    ...
    return c.getObjects().map(o => ({ type: o.type, role: o.data?.role, sel: o.selectable, w: Math.round(o.width||0), h: Math.round(o.height||0) }));
  });
  console.log("Objects on compact-quote canvas:");
