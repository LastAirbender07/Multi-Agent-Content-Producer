...
// ── Lumina light-theme background ────────────────────────────────────────────
// Shared factory for slides that need the Lumina airy background: a diagonal
// indigo→teal tint gradient + soft corner glows in theme colours.
// Used by aurora_content (Lumina path) and aurora_cta (Lumina path).
// Opacity and glow radii are intentionally subtle — they add depth without
// competing with the slide's text content.

export function createLuminaBg(t: CanvasTokens, height = CANVAS_SIZE): fabric.FabricObject[] {
  const gradRect = new fabric.Rect({
    ...
    fill: new fabric.Gradient({
      ...
      colorStops: [
        ...
        { offset: 0.5, color: "#F4F7FF" },  // near-white centre
        { offset: 1,   color: "#CCFBF1" },  // teal-100 — bottom-right
      ],
    }),
    selectable: false, evented: false, originX: "left" as const, originY: "top" as const,
  });
  setData(gradRect, { role: "lumina_bg" });
