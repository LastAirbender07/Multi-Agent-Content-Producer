...
export async function buildAuroraHook(
  ...
  const labelH    = 14 + 20;
  const headLines = Math.max(1, Math.ceil((slide.title?.length ?? 20) / 20));
  const headlineH = headLines * 72 * 1.05 + 28;
  const subLines  = slide.body ? Math.max(1, Math.ceil(slide.body.length / 38)) : 0;
