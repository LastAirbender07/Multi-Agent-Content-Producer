...
export async function buildAuroraLiteStat(
  slide: SlideData,
  ...
  if (slide.title) {
    const titleObj = makeText(slide.title, {
      role: "stat_title", fontSize: 18, fill: t.muted,  // aurora-lite: 18pt (was 15)
  ...
  // ── Body text ─────────────────────────────────────────────────────────────────
  if (slide.body) {
    const BODY_FS = 24;  // aurora-lite: 24pt (was 20) — Instagram standard 26-30pt
    ...
    const accentBar = new fabric.Rect({
      ...
      width: 3, height: rawH, rx: 2,
      fill: new fabric.Gradient({
        type: "linear", coords: { x1: 0, y1: 0, x2: 0, y2: rawH },
