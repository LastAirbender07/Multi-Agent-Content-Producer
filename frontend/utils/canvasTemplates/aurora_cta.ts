...
import { createBrandBar, createGlowBg, createPillButton, makeText, makeTitleText, createAccentLine } from "./shared";
...
export async function buildAuroraCta(
  ...
  if (dark) {
    ...
    // Lumina: soft radial spotlight effect (ambient glow) — pastel, centered.
    objects.push(...createGlowBg([
      { rx:580, ry:580, left: CS*0.30,  top:-CS*0.30,   color:t.secondary, opacity:0.45 },  // teal top-right
      { rx:540, ry:540, left:-CS*0.20,  top: CS*0.40,   color:t.primary,   opacity:0.40 },  // purple bottom-left
      { rx:200, ry:200, left: CS/2-100, top: CS/2-100,  color:t.primary,   opacity:0.10 },  // soft centre
    ]));
  }
